...
//
class cendereco extends cTable {
	var $id_endereco;
	var $endereco;
	var $bairro;
	var $estado;
	...
	function __construct() {
		...
		// id_endereco
		$this->id_endereco = new cField('endereco', 'endereco', 'x_id_endereco', 'id_endereco', '`id_endereco`', '`id_endereco`', 3, -1, FALSE, '`id_endereco`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// endereco
		$this->endereco = new cField('endereco', 'endereco', 'x_endereco', 'endereco', '`endereco`', '`endereco`', 200, -1, FALSE, '`endereco`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->endereco->Sortable = TRUE; // Allow sort
		$this->fields['endereco'] = &$this->endereco;

		// bairro
		$this->bairro = new cField('endereco', 'endereco', 'x_bairro', 'bairro', '`bairro`', '`bairro`', 200, -1, FALSE, '`bairro`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->bairro->Sortable = TRUE; // Allow sort
		$this->fields['bairro'] = &$this->bairro;

		// estado
		$this->estado = new cField('endereco', 'endereco', 'x_estado', 'estado', '`estado`', '`estado`', 200, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->estado->Sortable = TRUE; // Allow sort
		$this->fields['estado'] = &$this->estado;

		// cidade
		$this->cidade = new cField('endereco', 'endereco', 'x_cidade', 'cidade', '`cidade`', '`cidade`', 200, -1, FALSE, '`cidade`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cidade->Sortable = TRUE; // Allow sort
		$this->fields['cidade'] = &$this->cidade;

		// cep
		$this->cep = new cField('endereco', 'endereco', 'x_cep', 'cep', '`cep`', '`cep`', 3, -1, FALSE, '`cep`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cep->Sortable = TRUE; // Allow sort
		$this->cep->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`endereco`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->id_endereco->setDbValue($conn->Insert_ID());
			$rs['id_endereco'] = $this->id_endereco->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_endereco', $rs))
				ew_AddFilter($where, ew_QuotedName('id_endereco', $this->DBID) . '=' . ew_QuotedValue($rs['id_endereco'], $this->id_endereco->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->id_endereco->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->id_endereco->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@id_endereco@", ew_AdjustSql($this->id_endereco->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("enderecoview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("enderecoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "enderecoadd.php?" . $this->UrlParm($parm);
		else
			$url = "enderecoadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("enderecoedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("enderecoadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("enderecodelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "id_endereco:" . ew_VarToJson($this->id_endereco->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_endereco->CurrentValue)) {
			$sUrl .= "id_endereco=" . urlencode($this->id_endereco->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["id_endereco"]))
				$arKeys[] = $_POST["id_endereco"];
			elseif (isset($_GET["id_endereco"]))
				$arKeys[] = $_GET["id_endereco"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_endereco->CurrentValue = $key;
