...
class crepresentantes extends cTable {
	...
	function __construct() {
		...
		// Update Table
		$this->UpdateTable = "`representantes`";
		$this->DBID = 'DB';
		...
		// id_representantes
		$this->id_representantes = new cField('representantes', 'representantes', 'x_id_representantes', 'id_representantes', '`id_representantes`', '`id_representantes`', 3, -1, FALSE, '`id_representantes`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_representantes->Sortable = TRUE; // Allow sort
		$this->id_representantes->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "pessoa_fisica") {
			$sDetailUrl = $GLOBALS["pessoa_fisica"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_id_representantes=" . urlencode($this->id_representantes->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "representanteslist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`representantes`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->id_representantes->setDbValue($conn->Insert_ID());
			$rs['id_representantes'] = $this->id_representantes->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_representantes', $rs))
				ew_AddFilter($where, ew_QuotedName('id_representantes', $this->DBID) . '=' . ew_QuotedValue($rs['id_representantes'], $this->id_representantes->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->id_representantes->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->id_representantes->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@id_representantes@", ew_AdjustSql($this->id_representantes->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "representantesedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "representantesadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("representantesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("representantesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "representantesadd.php?" . $this->UrlParm($parm);
		else
			$url = "representantesadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("representantesedit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("representantesedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("representantesadd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("representantesadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "id_representantes:" . ew_VarToJson($this->id_representantes->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_representantes->CurrentValue)) {
			$sUrl .= "id_representantes=" . urlencode($this->id_representantes->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["id_representantes"]))
				$arKeys[] = $_POST["id_representantes"];
			elseif (isset($_GET["id_representantes"]))
				$arKeys[] = $_GET["id_representantes"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_representantes->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		$this->id_representantes->setDbValue($rs->fields('id_representantes'));
		$this->id_pessoa->setDbValue($rs->fields('id_pessoa'));
		$this->comissao->setDbValue($rs->fields('comissao'));
	}
	...
	function RenderEditRow() {
		...
		$this->comissao->EditValue = $this->comissao->CurrentValue;
