...
class ccategoria extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'categoria';
		$this->TableName = 'categoria';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`categoria`";
		$this->DBID = 'DB';
		...
		// id_categoria
		$this->id_categoria = new cField('categoria', 'categoria', 'x_id_categoria', 'id_categoria', '`id_categoria`', '`id_categoria`', 3, -1, FALSE, '`id_categoria`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// id_pai
		$this->id_pai = new cField('categoria', 'categoria', 'x_id_pai', 'id_pai', '`id_pai`', '`id_pai`', 3, -1, FALSE, '`id_pai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->id_pai->Sortable = TRUE; // Allow sort
		$this->id_pai->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->id_pai->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`categoria`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('id_categoria', $rs))
				ew_AddFilter($where, ew_QuotedName('id_categoria', $this->DBID) . '=' . ew_QuotedValue($rs['id_categoria'], $this->id_categoria->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->id_categoria->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->id_categoria->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@id_categoria@", ew_AdjustSql($this->id_categoria->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "categoriaedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "categoriaadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("categoriaview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("categoriaview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "categoriaadd.php?" . $this->UrlParm($parm);
		else
			$url = "categoriaadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("categoriaedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("categoriaadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "id_categoria:" . ew_VarToJson($this->id_categoria->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_categoria->CurrentValue)) {
			$sUrl .= "id_categoria=" . urlencode($this->id_categoria->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["id_categoria"]))
				$arKeys[] = $_POST["id_categoria"];
			elseif (isset($_GET["id_categoria"]))
				$arKeys[] = $_GET["id_categoria"];
			else
