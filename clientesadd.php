...
class cclientes_add extends cclientes {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "clientesview.php")
	...
	function RenderRow() {
		...
		$this->time->ViewCustomAttributes = "";
			...
			$sSqlWrk = "SELECT `id_pessoa`, `nome_pessoa` AS `DispFld`, `sobrenome_pessoa` AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `pessoa_fisica`";
			$sWhereWrk = "";
			$this->id->LookupFilters = array();
			ew_AddFilter($sWhereWrk, $sFilterWrk);
			$this->Lookup_Selecting($this->id, $sWhereWrk); // Call Lookup Selecting
	...
	function SetupLookupFilters($fld, $pageId = null) {
		...
		case "x_id":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id_pessoa` AS `LinkFld`, `nome_pessoa` AS `DispFld`, `sobrenome_pessoa` AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `pessoa_fisica`";
			$sWhereWrk = "";
...
var CurrentForm = fclientesadd = new ew_Form("fclientesadd", "add");
...
fclientesadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
fclientesadd.Lists["x_id"] = {"LinkField":"x_id_pessoa","Ajax":true,"AutoFill":false,"DisplayFields":["x_nome_pessoa","x_sobrenome_pessoa","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"pessoa_fisica"};
...
?>
<form name="fclientesadd" id="fclientesadd" class="<?php echo $clientes_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<?php if ($clientes->tipo->Visible) { // tipo ?>
	<div id="r_tipo" class="form-group">
		...
		<div class="<?php echo $clientes_add->RightColumnClass ?>"><div<?php echo $clientes->tipo->CellAttributes() ?>>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $clientes_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
