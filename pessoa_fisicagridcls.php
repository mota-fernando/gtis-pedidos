...
$pessoa_fisica_grid = NULL; // Initialize page object first

class cpessoa_fisica_grid extends cpessoa_fisica {
	...
	// Page object name
	var $PageObjName = 'pessoa_fisica_grid';

	// Grid form hidden field names
	var $FormName = 'fpessoa_fisicagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["pessoa_fisica"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->id_pessoa->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_nome_pessoa") && $objForm->HasValue("o_nome_pessoa") && $this->nome_pessoa->CurrentValue <> $this->nome_pessoa->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_sobrenome_pessoa") && $objForm->HasValue("o_sobrenome_pessoa") && $this->sobrenome_pessoa->CurrentValue <> $this->sobrenome_pessoa->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->id_pessoa->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->id_pessoa->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->nome_pessoa->setOldValue($objForm->GetValue("o_nome_pessoa"));
		if (!$this->sobrenome_pessoa->FldIsDetailKey) {
			$this->sobrenome_pessoa->setFormValue($objForm->GetValue("x_sobrenome_pessoa"));
		}
		$this->sobrenome_pessoa->setOldValue($objForm->GetValue("o_sobrenome_pessoa"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->id_pessoa->CurrentValue = strval($arKeys[0]); // id_pessoa
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "representantes") {
				$this->id_pessoa->CurrentValue = $this->id_pessoa->getSessionValue();
			}
		...
		if (strval($this->id_pessoa->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@id_representantes@", ew_AdjustSql($this->id_pessoa->CurrentValue, "DB"), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "representantes") {
			$this->id_pessoa->Visible = FALSE;
			if ($GLOBALS["representantes"]->EventCancelled) $this->EventCancelled = TRUE;
		}
