...
$representantes_delete = NULL; // Initialize page object first

class crepresentantes_delete extends crepresentantes {
	...
	// Page object name
	var $PageObjName = 'representantes_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("representanteslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in representantes class, representantesinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("representanteslist.php"); // Return to list
			}
		}
	}
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['id_representantes'];
				$conn->raiseErrorFn = $GLOBALS["EW_ERROR_FN"];
...
// Create page object
if (!isset($representantes_delete)) $representantes_delete = new crepresentantes_delete();

// Page init
$representantes_delete->Page_Init();

// Page main
$representantes_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$representantes_delete->Page_Render();
?>
...
var CurrentForm = frepresentantesdelete = new ew_Form("frepresentantesdelete", "delete");

// Form_CustomValidate event
frepresentantesdelete.Form_CustomValidate = 
...
<?php $representantes_delete->ShowPageHeader(); ?>
<?php
$representantes_delete->ShowMessage();
?>
<form name="frepresentantesdelete" id="frepresentantesdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($representantes_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $representantes_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="representantes">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($representantes_delete->RecKeys as $key) { ?>
...
<?php if ($representantes->id_representantes->Visible) { // id_representantes ?>
		<th class="<?php echo $representantes->id_representantes->HeaderCellClass() ?>"><span id="elh_representantes_id_representantes" class="representantes_id_representantes"><?php echo $representantes->id_representantes->FldCaption() ?></span></th>
<?php } ?>
...
<?php } ?>
	...
	<tbody>
<?php
$representantes_delete->RecCnt = 0;
$i = 0;
while (!$representantes_delete->Recordset->EOF) {
	$representantes_delete->RecCnt++;
	$representantes_delete->RowCnt++;

	// Set row properties
	$representantes->ResetAttrs();
	$representantes->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$representantes_delete->LoadRowValues($representantes_delete->Recordset);

	// Render row
	$representantes_delete->RenderRow();
?>
	<tr<?php echo $representantes->RowAttributes() ?>>
<?php if ($representantes->id_representantes->Visible) { // id_representantes ?>
		<td<?php echo $representantes->id_representantes->CellAttributes() ?>>
<span id="el<?php echo $representantes_delete->RowCnt ?>_representantes_id_representantes" class="representantes_id_representantes">
...
<?php
	$representantes_delete->Recordset->MoveNext();
}
$representantes_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $representantes_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
frepresentantesdelete.Init();
</script>
<?php
$representantes_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$representantes_delete->Page_Terminate();
?>
