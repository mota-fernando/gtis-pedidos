...
$tranportadora_edit = NULL; // Initialize page object first

class ctranportadora_edit extends ctranportadora {
	...
	// Page object name
	var $PageObjName = 'tranportadora_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			...
			// Load key from Form
			if ($objForm->HasValue("x_id_transportadora")) {
			...
			$loadByQuery = FALSE;
			if (isset($_GET["id_transportadora"])) {
	...
	function ValidateForm() {
		...
		if (in_array("empresas", $DetailTblVar) && $GLOBALS["empresas"]->DetailEdit) {
		...
		if ($rs->EOF) {
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("empresas", $DetailTblVar) && $GLOBALS["empresas"]->DetailEdit) {
						if (!isset($GLOBALS["empresas_grid"])) $GLOBALS["empresas_grid"] = new cempresas_grid(); // Get detail page object
						$EditRow = $GLOBALS["empresas_grid"]->GridUpdate();
					}
				}
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("empresas", $DetailTblVar)) {
				...
				if ($GLOBALS["empresas_grid"]->DetailEdit) {
					$GLOBALS["empresas_grid"]->CurrentMode = "edit";
					$GLOBALS["empresas_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($tranportadora_edit)) $tranportadora_edit = new ctranportadora_edit();

// Page init
$tranportadora_edit->Page_Init();

// Page main
$tranportadora_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tranportadora_edit->Page_Render();
?>
...
var CurrentForm = ftranportadoraedit = new ew_Form("ftranportadoraedit", "edit");

// Validate form
ftranportadoraedit.Validate = function() {
...
ftranportadoraedit.Form_CustomValidate = 
...
<?php $tranportadora_edit->ShowPageHeader(); ?>
<?php
$tranportadora_edit->ShowMessage();
?>
<form name="ftranportadoraedit" id="ftranportadoraedit" class="<?php echo $tranportadora_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tranportadora_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tranportadora_edit->Token ?>">
<?php } ?>
...
<?php
	if (in_array("empresas", explode(",", $tranportadora->getCurrentDetailTable())) && $empresas->DetailEdit) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tranportadora_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
ftranportadoraedit.Init();
</script>
<?php
$tranportadora_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tranportadora_edit->Page_Terminate();
?>
