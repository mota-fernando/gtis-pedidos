...
$detalhe_pedido_list = NULL; // Initialize page object first

class cdetalhe_pedido_list extends cdetalhe_pedido {
	...
	// Page object name
	var $PageObjName = 'detalhe_pedido_list';

	// Grid form hidden field names
	var $FormName = 'fdetalhe_pedidolist';
	...
	function __construct() {
		...
		$this->AddUrl = "detalhe_pedidoadd.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fdetalhe_pedidolistsrch";
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->id_detalhe->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->id_detalhe->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->id_detalhe->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fdetalhe_pedidolistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fdetalhe_pedidolistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fdetalhe_pedidolist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a class=\"ewExportLink ewPrint\" title=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\" onclick=\"ew_Export(document.fdetalhe_pedidolist,'" . ew_CurrentPage() . "','print',false,true);\">" . $Language->Phrase("PrinterFriendly") . "</a>";
		$item->Visible = TRUE;

		// Export to Excel
		$item = &$this->ExportOptions->Add("excel");
		$item->Body = "<a class=\"ewExportLink ewExcel\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\" onclick=\"ew_Export(document.fdetalhe_pedidolist,'" . ew_CurrentPage() . "','excel',false,true);\">" . $Language->Phrase("ExportToExcel") . "</a>";
		$item->Visible = TRUE;

		// Export to Word
		$item = &$this->ExportOptions->Add("word");
		$item->Body = "<a class=\"ewExportLink ewWord\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\" onclick=\"ew_Export(document.fdetalhe_pedidolist,'" . ew_CurrentPage() . "','word',false,true);\">" . $Language->Phrase("ExportToWord") . "</a>";
		$item->Visible = FALSE;

		// Export to Html
		$item = &$this->ExportOptions->Add("html");
		$item->Body = "<a class=\"ewExportLink ewHtml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\" onclick=\"ew_Export(document.fdetalhe_pedidolist,'" . ew_CurrentPage() . "','html',false,true);\">" . $Language->Phrase("ExportToHtml") . "</a>";
		$item->Visible = FALSE;

		// Export to Xml
		$item = &$this->ExportOptions->Add("xml");
		$item->Body = "<a class=\"ewExportLink ewXml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\" onclick=\"ew_Export(document.fdetalhe_pedidolist,'" . ew_CurrentPage() . "','xml',false,true);\">" . $Language->Phrase("ExportToXml") . "</a>";
		$item->Visible = FALSE;

		// Export to Csv
		$item = &$this->ExportOptions->Add("csv");
		$item->Body = "<a class=\"ewExportLink ewCsv\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\" onclick=\"ew_Export(document.fdetalhe_pedidolist,'" . ew_CurrentPage() . "','csv',false,true);\">" . $Language->Phrase("ExportToCsv") . "</a>";
		$item->Visible = FALSE;

		// Export to Pdf
		$item = &$this->ExportOptions->Add("pdf");
		$item->Body = "<a class=\"ewExportLink ewPdf\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\" onclick=\"ew_Export(document.fdetalhe_pedidolist,'" . ew_CurrentPage() . "','pdf',false,true);\">" . $Language->Phrase("ExportToPDF") . "</a>";
		$item->Visible = FALSE;

		// Export to Email
		$item = &$this->ExportOptions->Add("email");
		$url = "";
		$item->Body = "<button id=\"emf_detalhe_pedido\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_detalhe_pedido',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fdetalhe_pedidolist,sel:true" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "pedidos") {
			global $pedidos;
			if (!isset($pedidos)) $pedidos = new cpedidos;
			$rsmaster = $pedidos->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$Doc->Table = &$pedidos;
					$pedidos->ExportDocument($Doc, $rsmaster, 1, 1);
					$Doc->ExportEmptyRow();
					$Doc->Table = &$this;
				}
...
// Create page object
if (!isset($detalhe_pedido_list)) $detalhe_pedido_list = new cdetalhe_pedido_list();

// Page init
$detalhe_pedido_list->Page_Init();

// Page main
$detalhe_pedido_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$detalhe_pedido_list->Page_Render();
?>
...
var CurrentForm = fdetalhe_pedidolist = new ew_Form("fdetalhe_pedidolist", "list");
fdetalhe_pedidolist.FormKeyCountName = '<?php echo $detalhe_pedido_list->FormKeyCountName ?>';

// Form_CustomValidate event
fdetalhe_pedidolist.Form_CustomValidate = 
...
<?php if ($detalhe_pedido->Export == "") { ?>
<div class="ewToolbar">
<?php if ($detalhe_pedido_list->TotalRecs > 0 && $detalhe_pedido_list->ExportOptions->Visible()) { ?>
<?php $detalhe_pedido_list->ExportOptions->Render("body") ?>
<?php } ?>
...
<?php
	...
	if ($bSelectLimit) {
		if ($detalhe_pedido_list->TotalRecs <= 0)
			$detalhe_pedido_list->TotalRecs = $detalhe_pedido->ListRecordCount();
	} else {
		if (!$detalhe_pedido_list->Recordset && ($detalhe_pedido_list->Recordset = $detalhe_pedido_list->LoadRecordset()))
			$detalhe_pedido_list->TotalRecs = $detalhe_pedido_list->Recordset->RecordCount();
	}
	$detalhe_pedido_list->StartRec = 1;
	if ($detalhe_pedido_list->DisplayRecs <= 0 || ($detalhe_pedido->Export <> "" && $detalhe_pedido->ExportAll)) // Display all records
		$detalhe_pedido_list->DisplayRecs = $detalhe_pedido_list->TotalRecs;
	if (!($detalhe_pedido->Export <> "" && $detalhe_pedido->ExportAll))
		$detalhe_pedido_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$detalhe_pedido_list->Recordset = $detalhe_pedido_list->LoadRecordset($detalhe_pedido_list->StartRec-1, $detalhe_pedido_list->DisplayRecs);

	// Set no record found message
	if ($detalhe_pedido->CurrentAction == "" && $detalhe_pedido_list->TotalRecs == 0) {
		if ($detalhe_pedido_list->SearchWhere == "0=101")
			$detalhe_pedido_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$detalhe_pedido_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$detalhe_pedido_list->RenderOtherOptions();
?>
<?php $detalhe_pedido_list->ShowPageHeader(); ?>
<?php
$detalhe_pedido_list->ShowMessage();
?>
...
// Header row
$detalhe_pedido_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$detalhe_pedido_list->RenderListOptions();

// Render list options (header, left)
$detalhe_pedido_list->ListOptions->Render("header", "left");
?>
<?php if ($detalhe_pedido->id_detalhe->Visible) { // id_detalhe ?>
	...
	<?php } else { ?>
		<th data-name="id_detalhe" class="<?php echo $detalhe_pedido->id_detalhe->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $detalhe_pedido->SortUrl($detalhe_pedido->id_detalhe) ?>',1);"><div id="elh_detalhe_pedido_id_detalhe" class="detalhe_pedido_id_detalhe">
...
<?php if ($detalhe_pedido->numero_pedido->Visible) { // numero_pedido ?>
	...
	<?php } else { ?>
		<th data-name="numero_pedido" class="<?php echo $detalhe_pedido->numero_pedido->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $detalhe_pedido->SortUrl($detalhe_pedido->numero_pedido) ?>',1);"><div id="elh_detalhe_pedido_numero_pedido" class="detalhe_pedido_numero_pedido">
...
<?php if ($detalhe_pedido->id_produto->Visible) { // id_produto ?>
	...
	<?php } else { ?>
		<th data-name="id_produto" class="<?php echo $detalhe_pedido->id_produto->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $detalhe_pedido->SortUrl($detalhe_pedido->id_produto) ?>',1);"><div id="elh_detalhe_pedido_id_produto" class="detalhe_pedido_id_produto">
...
<?php if ($detalhe_pedido->quantidade->Visible) { // quantidade ?>
	...
	<?php } else { ?>
		<th data-name="quantidade" class="<?php echo $detalhe_pedido->quantidade->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $detalhe_pedido->SortUrl($detalhe_pedido->quantidade) ?>',1);"><div id="elh_detalhe_pedido_quantidade" class="detalhe_pedido_quantidade">
...
$detalhe_pedido_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($detalhe_pedido->ExportAll && $detalhe_pedido->Export <> "") {
	$detalhe_pedido_list->StopRec = $detalhe_pedido_list->TotalRecs;
} else {

	// Set the last record to display
	if ($detalhe_pedido_list->TotalRecs > $detalhe_pedido_list->StartRec + $detalhe_pedido_list->DisplayRecs - 1)
		$detalhe_pedido_list->StopRec = $detalhe_pedido_list->StartRec + $detalhe_pedido_list->DisplayRecs - 1;
	else
		$detalhe_pedido_list->StopRec = $detalhe_pedido_list->TotalRecs;
}
$detalhe_pedido_list->RecCnt = $detalhe_pedido_list->StartRec - 1;
if ($detalhe_pedido_list->Recordset && !$detalhe_pedido_list->Recordset->EOF) {
	$detalhe_pedido_list->Recordset->MoveFirst();
	$bSelectLimit = $detalhe_pedido_list->UseSelectLimit;
	if (!$bSelectLimit && $detalhe_pedido_list->StartRec > 1)
		$detalhe_pedido_list->Recordset->Move($detalhe_pedido_list->StartRec - 1);
} elseif (!$detalhe_pedido->AllowAddDeleteRow && $detalhe_pedido_list->StopRec == 0) {
	$detalhe_pedido_list->StopRec = $detalhe_pedido->GridAddRowCount;
}
...
while ($detalhe_pedido_list->RecCnt < $detalhe_pedido_list->StopRec) {
	$detalhe_pedido_list->RecCnt++;
	if (intval($detalhe_pedido_list->RecCnt) >= intval($detalhe_pedido_list->StartRec)) {
		$detalhe_pedido_list->RowCnt++;

		// Set up key count
		$detalhe_pedido_list->KeyCount = $detalhe_pedido_list->RowIndex;
		...
		if ($detalhe_pedido->CurrentAction == "gridadd") {
			...
			$detalhe_pedido_list->LoadRowValues($detalhe_pedido_list->Recordset); // Load row values
		}
		$detalhe_pedido->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$detalhe_pedido->RowAttrs = array_merge($detalhe_pedido->RowAttrs, array('data-rowindex'=>$detalhe_pedido_list->RowCnt, 'id'=>'r' . $detalhe_pedido_list->RowCnt . '_detalhe_pedido', 'data-rowtype'=>$detalhe_pedido->RowType));
...
$detalhe_pedido_list->ListOptions->Render("body", "left", $detalhe_pedido_list->RowCnt);
?>
	<?php if ($detalhe_pedido->id_detalhe->Visible) { // id_detalhe ?>
		<td data-name="id_detalhe"<?php echo $detalhe_pedido->id_detalhe->CellAttributes() ?>>
<span id="el<?php echo $detalhe_pedido_list->RowCnt ?>_detalhe_pedido_id_detalhe" class="detalhe_pedido_id_detalhe">
...
<span id="el<?php echo $detalhe_pedido_list->RowCnt ?>_detalhe_pedido_numero_pedido" class="detalhe_pedido_numero_pedido">
...
<span id="el<?php echo $detalhe_pedido_list->RowCnt ?>_detalhe_pedido_id_produto" class="detalhe_pedido_id_produto">
...
$detalhe_pedido_list->ListOptions->Render("body", "right", $detalhe_pedido_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($detalhe_pedido->CurrentAction <> "gridadd")
		$detalhe_pedido_list->Recordset->MoveNext();
}
?>
</tbody>
<?php
...
// Close recordset
if ($detalhe_pedido_list->Recordset)
	$detalhe_pedido_list->Recordset->Close();
?>
<?php if ($detalhe_pedido->Export == "") { ?>
...
<?php } ?>
<?php if ($detalhe_pedido_list->TotalRecs == 0 && $detalhe_pedido->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($detalhe_pedido_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($detalhe_pedido->Export == "") { ?>
<script type="text/javascript">
fdetalhe_pedidolist.Init();
</script>
<?php } ?>
<?php
$detalhe_pedido_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($detalhe_pedido->Export == "") { ?>
...
<?php
$detalhe_pedido_list->Page_Terminate();
?>
