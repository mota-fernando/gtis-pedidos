...
//
// Table class for tranportadora
//
class ctranportadora extends cTable {
	var $id_transportadora;
	var $transportadora;
	var $id_empresa_transportadora;
	...
	function __construct() {
		...
		$this->TableVar = 'tranportadora';
		$this->TableName = 'tranportadora';
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "empresas") {
			$sDetailUrl = $GLOBALS["empresas"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_id_transportadora=" . urlencode($this->id_transportadora->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "tranportadoralist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tranportadora`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->id_transportadora->setDbValue($conn->Insert_ID());
			$rs['id_transportadora'] = $this->id_transportadora->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_transportadora', $rs))
				ew_AddFilter($where, ew_QuotedName('id_transportadora', $this->DBID) . '=' . ew_QuotedValue($rs['id_transportadora'], $this->id_transportadora->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->id_transportadora->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->id_transportadora->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@id_transportadora@", ew_AdjustSql($this->id_transportadora->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "tranportadoraedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "tranportadoraadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tranportadoraview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tranportadoraview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tranportadoraadd.php?" . $this->UrlParm($parm);
		else
			$url = "tranportadoraadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tranportadoraedit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tranportadoraedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tranportadoraadd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tranportadoraadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tranportadoradelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "id_transportadora:" . ew_VarToJson($this->id_transportadora->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->id_transportadora->CurrentValue)) {
			$sUrl .= "id_transportadora=" . urlencode($this->id_transportadora->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["id_transportadora"]))
				$arKeys[] = $_POST["id_transportadora"];
			elseif (isset($_GET["id_transportadora"]))
				$arKeys[] = $_GET["id_transportadora"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->id_transportadora->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->transportadora->setDbValue($rs->fields('transportadora'));
		$this->id_empresa_transportadora->setDbValue($rs->fields('id_empresa_transportadora'));
	}
	...
	function RenderEditRow() {
		...
		$this->transportadora->EditValue = $this->transportadora->CurrentValue;
