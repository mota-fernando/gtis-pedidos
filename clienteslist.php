...
class cclientes_list extends cclientes {
	...
	function __construct() {
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fclienteslistsrch";
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fclienteslist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
var CurrentForm = fclienteslist = new ew_Form("fclienteslist", "list");
...
<?php
	...
	if ($bSelectLimit) {
		if ($clientes_list->TotalRecs <= 0)
			$clientes_list->TotalRecs = $clientes->ListRecordCount();
	} else {
...
<div class="box ewBox ewGrid<?php if ($clientes_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> clientes">
...
// Header row
$clientes_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if (!isset($clientes_list->Pager)) $clientes_list->Pager = new cPrevNextPager($clientes_list->StartRec, $clientes_list->DisplayRecs, $clientes_list->TotalRecs, $clientes_list->AutoHidePager) ?>
