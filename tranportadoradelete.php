...
$tranportadora_delete = NULL; // Initialize page object first

class ctranportadora_delete extends ctranportadora {
	...
	// Page object name
	var $PageObjName = 'tranportadora_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("tranportadoralist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in tranportadora class, tranportadorainfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("tranportadoralist.php"); // Return to list
			}
		}
	}
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['id_transportadora'];
				$conn->raiseErrorFn = $GLOBALS["EW_ERROR_FN"];
...
// Create page object
if (!isset($tranportadora_delete)) $tranportadora_delete = new ctranportadora_delete();

// Page init
$tranportadora_delete->Page_Init();

// Page main
$tranportadora_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tranportadora_delete->Page_Render();
?>
...
var CurrentForm = ftranportadoradelete = new ew_Form("ftranportadoradelete", "delete");

// Form_CustomValidate event
ftranportadoradelete.Form_CustomValidate = 
...
<?php $tranportadora_delete->ShowPageHeader(); ?>
<?php
$tranportadora_delete->ShowMessage();
?>
<form name="ftranportadoradelete" id="ftranportadoradelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tranportadora_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tranportadora_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tranportadora">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tranportadora_delete->RecKeys as $key) { ?>
...
<?php } ?>
	...
	<tbody>
<?php
$tranportadora_delete->RecCnt = 0;
$i = 0;
while (!$tranportadora_delete->Recordset->EOF) {
	$tranportadora_delete->RecCnt++;
	$tranportadora_delete->RowCnt++;

	// Set row properties
	$tranportadora->ResetAttrs();
	$tranportadora->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tranportadora_delete->LoadRowValues($tranportadora_delete->Recordset);
...
<?php
	$tranportadora_delete->Recordset->MoveNext();
}
$tranportadora_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tranportadora_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftranportadoradelete.Init();
</script>
<?php
$tranportadora_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tranportadora_delete->Page_Terminate();
?>
