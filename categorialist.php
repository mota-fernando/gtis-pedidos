...
$categoria_list = NULL; // Initialize page object first

class ccategoria_list extends ccategoria {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["categoria"]) || get_class($GLOBALS["categoria"]) == "ccategoria") {
			$GLOBALS["categoria"] = &$this;
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fcategorialistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $categoria;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($categoria);
				$doc->Text = $sContent;
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->id_categoria->setFormValue($arrKeyFlds[0]);
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fcategorialistsrch", $filters);
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->id_categoria->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fcategorialistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fcategorialistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fcategorialist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fcategorialistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a class=\"ewExportLink ewPrint\" title=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("PrinterFriendlyText")) . "\" onclick=\"ew_Export(document.fcategorialist,'" . ew_CurrentPage() . "','print',false,true);\">" . $Language->Phrase("PrinterFriendly") . "</a>";
		$item->Visible = TRUE;

		// Export to Excel
		$item = &$this->ExportOptions->Add("excel");
		$item->Body = "<a class=\"ewExportLink ewExcel\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToExcelText")) . "\" onclick=\"ew_Export(document.fcategorialist,'" . ew_CurrentPage() . "','excel',false,true);\">" . $Language->Phrase("ExportToExcel") . "</a>";
		$item->Visible = TRUE;

		// Export to Word
		$item = &$this->ExportOptions->Add("word");
		$item->Body = "<a class=\"ewExportLink ewWord\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToWordText")) . "\" onclick=\"ew_Export(document.fcategorialist,'" . ew_CurrentPage() . "','word',false,true);\">" . $Language->Phrase("ExportToWord") . "</a>";
		$item->Visible = FALSE;

		// Export to Html
		$item = &$this->ExportOptions->Add("html");
		$item->Body = "<a class=\"ewExportLink ewHtml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToHtmlText")) . "\" onclick=\"ew_Export(document.fcategorialist,'" . ew_CurrentPage() . "','html',false,true);\">" . $Language->Phrase("ExportToHtml") . "</a>";
		$item->Visible = FALSE;

		// Export to Xml
		$item = &$this->ExportOptions->Add("xml");
		$item->Body = "<a class=\"ewExportLink ewXml\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToXmlText")) . "\" onclick=\"ew_Export(document.fcategorialist,'" . ew_CurrentPage() . "','xml',false,true);\">" . $Language->Phrase("ExportToXml") . "</a>";
		$item->Visible = FALSE;

		// Export to Csv
		$item = &$this->ExportOptions->Add("csv");
		$item->Body = "<a class=\"ewExportLink ewCsv\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToCsvText")) . "\" onclick=\"ew_Export(document.fcategorialist,'" . ew_CurrentPage() . "','csv',false,true);\">" . $Language->Phrase("ExportToCsv") . "</a>";
		$item->Visible = FALSE;

		// Export to Pdf
		$item = &$this->ExportOptions->Add("pdf");
		$item->Body = "<a class=\"ewExportLink ewPdf\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\" data-caption=\"" . ew_HtmlEncode($Language->Phrase("ExportToPDFText")) . "\" onclick=\"ew_Export(document.fcategorialist,'" . ew_CurrentPage() . "','pdf',false,true);\">" . $Language->Phrase("ExportToPDF") . "</a>";
		$item->Visible = FALSE;

		// Export to Email
		$item = &$this->ExportOptions->Add("email");
		$url = "";
		$item->Body = "<button id=\"emf_categoria\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_categoria',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fcategorialist,sel:true" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($categoria_list)) $categoria_list = new ccategoria_list();
...
var CurrentForm = fcategorialist = new ew_Form("fcategorialist", "list");
fcategorialist.FormKeyCountName = '<?php echo $categoria_list->FormKeyCountName ?>';

// Form_CustomValidate event
fcategorialist.Form_CustomValidate = 
...
<?php if ($categoria->Export == "") { ?>
<div class="ewToolbar">
<?php if ($categoria_list->TotalRecs > 0 && $categoria_list->ExportOptions->Visible()) { ?>
<?php $categoria_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($categoria_list->SearchOptions->Visible()) { ?>
<?php $categoria_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($categoria_list->FilterOptions->Visible()) { ?>
<?php $categoria_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $categoria_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($categoria_list->TotalRecs <= 0)
			$categoria_list->TotalRecs = $categoria->ListRecordCount();
	} else {
		if (!$categoria_list->Recordset && ($categoria_list->Recordset = $categoria_list->LoadRecordset()))
	...
	if ($categoria->CurrentAction == "" && $categoria_list->TotalRecs == 0) {
		if ($categoria_list->SearchWhere == "0=101")
			$categoria_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$categoria_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$categoria_list->RenderOtherOptions();
?>
<?php if ($categoria->Export == "" && $categoria->CurrentAction == "") { ?>
<form name="fcategorialistsrch" id="fcategorialistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($categoria_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fcategorialistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="categoria">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($categoria_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($categoria_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $categoria_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($categoria_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($categoria_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($categoria_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($categoria_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<div id="gmp_categoria" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($categoria_list->TotalRecs > 0 || $categoria->CurrentAction == "gridedit") { ?>
<table id="tbl_categorialist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$categoria_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($categoria->id_categoria->Visible) { // id_categoria ?>
	<?php if ($categoria->SortUrl($categoria->id_categoria) == "") { ?>
		<th data-name="id_categoria" class="<?php echo $categoria->id_categoria->HeaderCellClass() ?>"><div id="elh_categoria_id_categoria" class="categoria_id_categoria"><div class="ewTableHeaderCaption"><?php echo $categoria->id_categoria->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id_categoria" class="<?php echo $categoria->id_categoria->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $categoria->SortUrl($categoria->id_categoria) ?>',1);"><div id="elh_categoria_id_categoria" class="categoria_id_categoria">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $categoria->id_categoria->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($categoria->id_categoria->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($categoria->id_categoria->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
<?php if ($categoria->id_pai->Visible) { // id_pai ?>
	<?php if ($categoria->SortUrl($categoria->id_pai) == "") { ?>
		<th data-name="id_pai" class="<?php echo $categoria->id_pai->HeaderCellClass() ?>"><div id="elh_categoria_id_pai" class="categoria_id_pai"><div class="ewTableHeaderCaption"><?php echo $categoria->id_pai->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id_pai" class="<?php echo $categoria->id_pai->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $categoria->SortUrl($categoria->id_pai) ?>',1);"><div id="elh_categoria_id_pai" class="categoria_id_pai">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $categoria->id_pai->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($categoria->id_pai->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($categoria->id_pai->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
while ($categoria_list->RecCnt < $categoria_list->StopRec) {
	...
	if (intval($categoria_list->RecCnt) >= intval($categoria_list->StartRec)) {
		...
		$categoria->RowAttrs = array_merge($categoria->RowAttrs, array('data-rowindex'=>$categoria_list->RowCnt, 'id'=>'r' . $categoria_list->RowCnt . '_categoria', 'data-rowtype'=>$categoria->RowType));
...
$categoria_list->ListOptions->Render("body", "left", $categoria_list->RowCnt);
?>
	<?php if ($categoria->id_categoria->Visible) { // id_categoria ?>
		<td data-name="id_categoria"<?php echo $categoria->id_categoria->CellAttributes() ?>>
<span id="el<?php echo $categoria_list->RowCnt ?>_categoria_id_categoria" class="categoria_id_categoria">
...
<span id="el<?php echo $categoria_list->RowCnt ?>_categoria_id_pai" class="categoria_id_pai">
...
$categoria_list->ListOptions->Render("body", "right", $categoria_list->RowCnt);
?>
	</tr>
<?php
	}
...
<?php } ?>
<?php if ($categoria->CurrentAction == "") { ?>
...
<?php } ?>
<?php if ($categoria_list->TotalRecs == 0 && $categoria->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($categoria_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($categoria->Export == "") { ?>
<script type="text/javascript">
fcategorialistsrch.FilterList = <?php echo $categoria_list->GetFilterList() ?>;
fcategorialistsrch.Init();
fcategorialist.Init();
</script>
<?php } ?>
<?php
