...
$detalhe_pedido_grid = NULL; // Initialize page object first

class cdetalhe_pedido_grid extends cdetalhe_pedido {
	...
	// Page object name
	var $PageObjName = 'detalhe_pedido_grid';

	// Grid form hidden field names
	var $FormName = 'fdetalhe_pedidogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["detalhe_pedido"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->id_detalhe->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_numero_pedido") && $objForm->HasValue("o_numero_pedido") && $this->numero_pedido->CurrentValue <> $this->numero_pedido->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_id_produto") && $objForm->HasValue("o_id_produto") && $this->id_produto->CurrentValue <> $this->id_produto->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_desconto") && $objForm->HasValue("o_desconto") && $this->desconto->CurrentValue <> $this->desconto->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_preco") && $objForm->HasValue("o_preco") && $this->preco->CurrentValue <> $this->preco->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_quantidade") && $objForm->HasValue("o_quantidade") && $this->quantidade->CurrentValue <> $this->quantidade->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_subtotal") && $objForm->HasValue("o_subtotal") && $this->subtotal->CurrentValue <> $this->subtotal->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->id_detalhe->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->desconto->OldValue = $this->desconto->CurrentValue;
		$this->preco->CurrentValue = 0 ;
		$this->preco->OldValue = $this->preco->CurrentValue;
	...
	function LoadFormValues() {
		...
		if (!$this->id_detalhe->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->id_detalhe->CurrentValue = strval($arKeys[0]); // id_detalhe
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		$this->subtotal->ViewCustomAttributes = "";
			...
			$this->subtotal->EditValue = ew_FormatNumber($this->subtotal->EditValue, -2, -1, -2, 0);
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "pedidos") {
			...
			if ($GLOBALS["pedidos"]->EventCancelled) $this->EventCancelled = TRUE;
		}
